...
public class customerverification extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			PreparedStatement ps=con.prepareStatement("select * from customer_reg where firstname='"+username+"'");
			...
			while(res.next()){
				if(res.getString(3).equals(username) && res.getString(6).equals(password) ){
					if(res.getString(12).equals("pending")){
						...
						flag=1;
						String customerphno = "+91"+res.getString(5);
					    ...
					    Message message = Message
				                .creator(new PhoneNumber(customerphno), // to
					...
					response.sendRedirect("customerloginform.html");
				}
			}
			if(flag==0){
				response.sendRedirect("customerloginform.html");
